...
logger = logging.getLogger("RepublicScraper")
...
def scrape_republic_india() -> list:
    """
    Scrapes the Republic World India News section using the public RSS feed.
    ...
    """
    url = "https://www.republicworld.com/rss/india.xml"
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
    }

    logger.info(f"Fetching Republic India RSS feed from: {url}")
    try:
        response = requests.get(url, headers=headers, timeout=15)
        response.raise_for_status()
    except Exception as e:
        logger.error(f"Failed to fetch Republic RSS feed: {e}")
        return []
    ...
    for index, item in enumerate(items):
        ...
        # Clean special quote characters or formatting if present in Republic headlines
        if headline:
            # Republic's RSS feed injects non-printable/private-use unicode characters around every letter
            # We strip non-ascii printable characters, and literal triple quotes just in case
            headline = re.sub(r"[^\x20-\x7E]", "", headline).replace("'''", "")
        ...
        if db is not None:
            try:
                # Generate unique document ID based on URL hash to prevent duplicates
        ...
        # To prevent long-running test scripts if the feed contains too many entries (e.g. 50+),
        # let's limit it to 20 articles so it doesn't timeout the user console.
        if len(scraped_articles) >= 20:
            logger.info("Reached target batch limit of 20 articles. Stopping collection.")
            break
...
if __name__ == "__main__":
    articles = scrape_republic_india()
