...
logger = logging.getLogger("IEScraper")
...
def scrape_ie_india() -> list:
    """
    Scrapes Indian Express India News section using the public RSS feed.
    ...
    logger.info(f"Fetching Indian Express India RSS feed from: {url}")
    try:
        response = requests.get(url, headers=headers, timeout=15)
        response.raise_for_status()
    except Exception as e:
        logger.error(f"Failed to fetch Indian Express RSS feed: {e}")
        return []
    ...
    for index, item in enumerate(items):
        ...
        # Normalize timestamp to ISO format (IE uses RFC 2822 with UTC offset)
        timestamp = pub_date
        if pub_date:
            try:
