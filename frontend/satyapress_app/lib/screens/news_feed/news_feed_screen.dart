...
/// NewsFeedScreen shows a scrollable feed of news articles with search and filtering.
...
class _NewsFeedScreenState extends State<NewsFeedScreen> {
  ...
  final List<String> _categories = ['All', 'Tech', 'Global', 'Infrastructure', 'General'];

  List<String> get _dynamicSources {
    final Set<String> sources = {};
    for (final article in _apiArticles) {
      if (article.sourceName.trim().isNotEmpty) {
        sources.add(article.sourceName.trim());
      }
    }
    final List<String> sortedList = sources.toList()..sort();
  ...
  Future<void> _loadArticles() async {
      ...
      try {
        ...
        parsedArticles.add(
          NewsArticle(
            ...
            category: (article['category']?.toString() ?? '').isNotEmpty
                ? article['category'].toString()
                : _classifyCategory(title),
    ...
    if (mounted) {
      setState(() {
        _apiArticles = parsedArticles.isNotEmpty ? parsedArticles : MockNewsDatabase.articles;
        _isLoading = false;
      });
    }
  }
  ...
  List<NewsArticle> get _filteredArticles {
    ...
    return _apiArticles.where((article) {
      ...
      final matchesCategory =
          _selectedCategory == 'All' || article.category.toLowerCase() == _selectedCategory.toLowerCase();

      final matchesSource = _selectedSource == 'All' ||
          article.sourceName.trim() == _selectedSource.trim();
            ...
            child: Column(
              ...
              children: [
                ...
                SingleChildScrollView(
                  ...
                  padding: const EdgeInsets.symmetric(horizontal: 12.0),
                  child: Row(
                    children: _dynamicSources.map((source) {
                      return Padding(
  ...
  String _classifyCategory(String headline) {
    final headlineLower = headline.toLowerCase();
    
    final techKeywords = ['tech', 'digital', 'ai', 'cyber', 'internet', 'software'];
    final globalKeywords = ['world', 'global', 'international', 'china', 'pakistan', 'russia'];
    final infraKeywords = ['road', 'highway', 'bridge', 'infrastructure', 'construction', 'corridor'];
    
    if (techKeywords.any((kw) => headlineLower.contains(kw))) {
      return 'Tech';
    }
    
    if (globalKeywords.any((kw) => headlineLower.contains(kw)) ||
        RegExp(r'\bUS\b|\bU\.S\.\b').hasMatch(headline)) {
      return 'Global';
    }
    
    if (infraKeywords.any((kw) => headlineLower.contains(kw))) {
